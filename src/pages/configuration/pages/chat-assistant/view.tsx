...
export const PageView = () => {
  ...
  const [_chatAssistantQuickLink, chatAssistantQuickLinkError] = QuickLinkKubeObject.useGet(
    SYSTEM_QUICK_LINKS.CODEMIE,
    getDefaultNamespace()
  );

  const chatAssistantQuickLink = _chatAssistantQuickLink?.jsonData;
  const mode = !!chatAssistantSecret ? FORM_MODES.EDIT : FORM_MODES.CREATE;

  const error = chatAssistantSecretsError || chatAssistantQuickLinkError;
  const isLoading = (chatAssistantSecrets === null || chatAssistantQuickLink === null) && !error;
  ...
  const renderPageContent = React.useCallback(() => {
    ...
    if (!chatAssistantSecret && !isLoading) {
      return (
        <>
          <EmptyList
    ...
    const connected =
      chatAssistantSecret?.metadata?.annotations?.[SECRET_ANNOTATION_INTEGRATION_SECRET_CONNECTED];
    const statusError =
      chatAssistantSecret?.metadata?.annotations?.[SECRET_ANNOTATION_INTEGRATION_SECRET_ERROR];
