...
export const ManageArgoCDCI = ({ formData }: ManageArgoCDIntegrationSecretProps) => {
  const { argoCDSecret, mode, ownerReference } = formData;
  ...
  const error = argoCDSecret?.metadata?.annotations?.[SECRET_ANNOTATION_INTEGRATION_SECRET_ERROR];
  ...
  return (
    <Grid container spacing={2} data-testid="form">
      {mode === FORM_MODES.CREATE ? (
        <Grid item xs={12}>
          <CreateItemAccordion
            isExpanded={expandedPanel === mode || !argoCDSecret}
